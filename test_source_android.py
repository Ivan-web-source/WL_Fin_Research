...
def check_play_store():
    url = f"https://play.google.com/store/apps/details?id={PRIMARY['pkg']}&hl=en"
    r = safe_get(url)
...
def check_apkmirror_list():
    url = f"https://www.apkmirror.com/apk/{PRIMARY['apkmirror_slug']}/"
    r = safe_get(url)
    if not r or r.status_code != 200:
    ...
    record("2_APKMirror_list", True, len(set(ver_entries)) or vers_in(raw), d > 0, False,
           f"{len(set(ver_entries))} vers with Android tag | {d} dates | page_len={len(raw)}", url=url)


def check_apkmirror_paged():
    url = f"https://www.apkmirror.com/apk/{PRIMARY['apkmirror_slug']}/page/2/"
    r = safe_get(url)
    if not r or r.status_code != 200:
    ...
    record("3_APKMirror_page2", True, len(set(ver_entries)), d > 0, False,
           ...
           "Pagination works — scrape N pages for full history", url=url)


def check_apkmirror_release():
    list_url = f"https://www.apkmirror.com/apk/{PRIMARY['apkmirror_slug']}/"
    r = safe_get(list_url)
    if not r or r.status_code != 200:
    ...
    if not link or not link.get("href"):
        record("4_APKMirror_release", True, 0, False, False, "No release link on list page", url=list_url)
        return
...
def check_apkpure_list():
    url = f"https://apkpure.com/{PRIMARY['apkpure_slug']}/{PRIMARY['pkg']}/versions"
    r = safe_get(url)
    if not r or r.status_code != 200:
...
def check_apkpure_main():
    url = f"https://apkpure.com/{PRIMARY['apkpure_slug']}/{PRIMARY['pkg']}"
    r = safe_get(url)
    if not r or r.status_code != 200:
...
def check_apptopia_android():
    url = f"https://apptopia.com/google-play/app/{PRIMARY['pkg']}/about"
    r = safe_get(url)
    if not r or r.status_code != 200:
...
def check_appbrain():
    url = f"https://www.appbrain.com/app/{PRIMARY['apkpure_slug']}/{PRIMARY['pkg']}"
    r = safe_get(url)
    if not r or r.status_code != 200:
...
def check_appbrain_history():
    url = f"https://www.appbrain.com/app/{PRIMARY['apkpure_slug']}/{PRIMARY['pkg']}/history"
    r = safe_get(url)
    if not r or r.status_code != 200:
...
def check_play_json():
    url = f"https://play.google.com/store/apps/details?id={PRIMARY['pkg']}&hl=en&gl=us"
    ...
    record("10_Play_JSON_endpoint", r.status_code == 200, 1 if ver_m else 0, False, notes_in(raw),
           ...
           "Same as Play Store HTML — single version only", url=url)


def check_wayback_play_cdx():
    url = (
        f"http://web.archive.org/cdx/search/cdx"
        f"?url=play.google.com/store/apps/details%3Fid%3D{PRIMARY['pkg']}"
    ...
    try:
        ...
        record("11_Wayback_Play_CDX", True, 0, len(stamps) > 0, False,
               ...
               "Fetch each snapshot to extract version data", url=url)
    except Exception as e:
        record("11_Wayback_Play_CDX", True, 0, False, False, f"Parse err: {e}", url=url)


def check_wayback_play_snap():
    snap_url = f"https://web.archive.org/web/20230601120000/https://play.google.com/store/apps/details?id={PRIMARY['pkg']}&hl=en"
    r = safe_get(snap_url)
    ...
    record("12_Wayback_Play_snap", True,
           ...
           "Single snapshot test — scale for full history", url=snap_url)
...
def check_apkmirror_whatsnew():
    ...
    record("13_APKMirror_whatsnew", True, vers_in(r.text), dates_in(r.text) > 0, bool(wn_blocks),
           f"{len(wn_blocks)} what's-new blocks | sample: {wn_text[:80]}",
           "Notes on individual release pages — need to follow links", url=list_url)


def check_appfollow_android():
    url = f"https://appfollow.io/apps/{PRIMARY['apkpure_slug']}/android/{PRIMARY['pkg']}"
    r = safe_get(url)
    if not r:
...
def check_appshopper_android():
    url = f"https://appshopper.com/android/app/{PRIMARY['apkpure_slug']}/{PRIMARY['pkg']}"
    r = safe_get(url)
    if not r:
...
def check_play_internal_api():
    ...
    body = f'f.req=%5B%5B%5B%22xdSrCf%22%2C%22%5B%5B%5B%5C%22{PRIMARY["pkg"]}%5C%22%2C7%5D%5D%5D%22%2Cnull%2C%221%22%5D%5D%5D'
    try:
...
def check_apkcombo():
    url = f"https://apkcombo.com/apk-downloader/?package={PRIMARY['pkg']}"
    r = safe_get(url)
    if not r:
...
def check_apkfab():
    url = f"https://apkfab.com/apk/{PRIMARY['apkpure_slug']}/{PRIMARY['pkg']}"
    r = safe_get(url)
    if not r:
...
def check_apkfab_versions():
    url = f"https://apkfab.com/apk/{PRIMARY['apkpure_slug']}/{PRIMARY['pkg']}/versions"
    r = safe_get(url)
    if not r:
...
def check_all_apps_on_winners():
    print("\n\n" + "─" * 66)
    print("  Multi-app spot check: APKMirror + APKPure on all apps")
    print("─" * 66)
    for app in ANDROID_APPS:
        ...
        for label, url in [
            ...
            if r and r.status_code == 200:
                results[label] = f"v={vers_in(r.text)}, d={dates_in(r.text)}"
            else:
...
if __name__ == "__main__":
    print(f"Android Source Diagnostic")
    print(f"App: {PRIMARY['name']} ({PRIMARY['pkg']})")
    ...
    checks = [
        ...
        ("Apptopia",                check_apptopia_android),
    ...
    print(f"\nSaved to: source_diagnostic_android.json")